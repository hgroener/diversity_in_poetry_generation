...
import gc
import pickle
from transformers import EarlyStoppingCallback, TrainerCallback, DataCollatorForLanguageModeling
...
from transformers import AutoModelForCausalLM, TrainingArguments, AutoTokenizer, pipeline, logging, Trainer#, BitsAndBytesConfig
...
def load_model(model_name="meta-llama/Llama-2-7b-hf", instruct=False, conditioned=False, bp_and_tags=True):
    ...
    elif 'bygpt5' in model_name.lower():
        tokenizer = ByGPT5Tokenizer.from_pretrained(model_name)
...
def get_train_args(args):
    training_params = TrainingArguments(output_dir=args.out_dir, num_train_epochs=args.epochs, per_device_train_batch_size=args.batch_size,
                                        gradient_accumulation_steps=args.gradient_accumulation_steps, optim="adamw_hf", save_strategy='epoch',
...
def create_prompt(df_row, lang: str, conditioned=False, detokenizer=None, tokenizer=None, bp_and_tags=True) -> str:
    if detokenizer:
...
class CustomCallback(TrainerCallback):
    def on_epoch_end(self, args, state, control, **kwargs):
        ...
        if "bygpt5" in str(model.config_class).lower():
            pipe = TextGenerationPipeline(model=model,tokenizer=tokenizer, device=torch.device("cuda:0"))
        else:
...
def load_data(path, lang, conditioned, tokenizer, instruct=False, mode='debug', bp_and_tags=True):
    ...
    if instruct:
        ...
        df['text'] = df.apply(lambda x: create_prompt(x, l2l[lang], conditioned, detokenizer, tokenizer, bp_and_tags), axis=1)
    if detokenizer:
        detokenizer.close()
...
def tokenize_function(dataset, tokenizer=None):
...
def main(args):
    ...
    if args.bp_and_tags:
        ...
        BASIC_PROMPT = tokenizer.bos_token
        #BASIC_PROMPT = "Write a quatrain below:\n"

    dataset = load_data(path.format(args.data_dir, args.lang, 'train', 100), args.lang, args.conditioned, tokenizer, args.instruct, args.mode, args.bp_and_tags)
    eval_dataset = load_data(path.format(args.data_dir, args.lang, 'devtest', 100), args.lang, args.conditioned, tokenizer, args.instruct, args.mode, args.bp_and_tags)
    if "bygpt5" in args.model_name.lower():
    ...
    print(callbacks)
    if "bygpt5" in args.model_name.lower():
        trainer = Trainer(model=model, train_dataset=dataset, tokenizer=tokenizer, args=training_params,
                          eval_dataset=eval_dataset, callbacks=callbacks, data_collator=data_collator)
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--bp_and_tags', action='store_false', help='use prompt and <quatrain> tags or not?')
